...
class WXTool {
  ...
  */
  ...
  public static function wxPayData($body, $out_trade_no, $total_fee, $openid) {
    ...
    $notify_url = 'http://' . $_SERVER['HTTP_HOST'] . '/pay/wx_notify';
    ...
    $wx_pay_config->timestamp = $timeStamp;
    $wx_pay_config->nonceStr = $nonceStr;
    $wx_pay_config->package = $package;
    $wx_pay_config->signType = $signType;
    $wx_pay_config->paySign = $sign;

    $wx_pay_config->status = 0;
    $wx_pay_config->message = '返回成功';

    Log::info($wx_pay_config->toJson());

    return $wx_pay_config->toJson();
  }
}
